...
class GetFilesFromFolderViewHelper extends AbstractViewHelper {
    public function initializeArguments() {
        $this->registerArgument('data', 'array', 'Uid of the record which has the referenced records', true);
        $this->registerArgument('folder', 'string', 'Uid of the record which has the referenced records', true);
    }
    ...
    public static function renderStatic(array $arguments, \Closure $renderChildrenClosure, RenderingContextInterface $renderingContext) {
        ...
        $folderConfig = \explode(':', $arguments['data'][$arguments['folder']]);
        $storageRepository = GeneralUtility::makeInstance(StorageRepository::class);
        $storage = $storageRepository->findByUid($folderConfig[0]);
        $folder = $storage->getFolder($folderConfig[1]);
